  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={() => { setForm(emptyForm); setEditing(false); setShowForm(true) }} className="bg-luxury-gold text-luxury-900 px-4 py-2 rounded font-semibold hover:bg-luxury-gold-light transition-colors">+ Add Apartment</button>
      </div>
      <div className="bg-luxury-800 border border-luxury-700 rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          <thead><tr className="bg-luxury-700 text-white/70"><th className="text-left p-3">Name</th><th className="text-left p-3">Type</th><th className="text-left p-3">Price</th><th className="text-left p-3">Available</th><th className="text-right p-3">Actions</th></tr></thead>
          <tbody>{apartments.map((a) => (
            <tr key={a.id} className="border-t border-luxury-700"><td className="p-3 font-medium">{a.name}</td><td className="p-3"><span className="bg-luxury-gold/20 text-luxury-gold text-xs px-2 py-0.5 rounded">{a.type}</span></td><td className="p-3 text-white/70">{a.price}</td><td className="p-3">{a.available ? <span className="text-green-400">Yes</span> : <span className="text-red-400">No</span>}</td><td className="p-3 text-right"><button onClick={() => handleEdit(a)} className="text-luxury-gold hover:text-luxury-gold-light mr-3">Edit</button><button onClick={() => handleDelete(a.id)} className="text-red-400 hover:text-red-300">Delete</button></td></tr>
          ))}</tbody>
        </table>
      </div>
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4">
          <div className="bg-luxury-800 border border-luxury-700 rounded-xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6">
            ...
            <div className="space-y-3">
              ...
              <div><label className="block text-xs text-white/70 mb-1">Type</label><select value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value })} className="w-full bg-luxury-900 border border-luxury-600 rounded px-3 py-2 text-white text-sm"><option value="studio">Studio</option><option value="1-bed">1-Bed</option><option value="2-bed">2-Bed</option><option value="3-bed">3-Bed</option></select></div>
              <div className="grid grid-cols-3 gap-3">
                <div><label className="block text-xs text-white/70 mb-1">Beds</label><input type="number" value={form.beds} onChange={(e) => setForm({ ...form, beds: Number(e.target.value) })} className="w-full bg-luxury-900 border border-luxury-600 rounded px-3 py-2 text-white text-sm" /></div>
                <div><label className="block text-xs text-white/70 mb-1">Baths</label><input type="number" value={form.baths} onChange={(e) => setForm({ ...form, baths: Number(e.target.value) })} className="w-full bg-luxury-900 border border-luxury-600 rounded px-3 py-2 text-white text-sm" /></div>
                <div><label className="block text-xs text-white/70 mb-1">Size (sqm)</label><input type="number" value={form.size_sqm} onChange={(e) => setForm({ ...form, size_sqm: Number(e.target.value) })} className="w-full bg-luxury-900 border border-luxury-600 rounded px-3 py-2 text-white text-sm" /></div>
              </div>
              <div><label className="block text-xs text-white/70 mb-1">Amenities (comma separated)</label><input value={amenitiesInput} onChange={(e) => { setAmenitiesInput(e.target.value); setForm({ ...form, amenities: e.target.value.split(',').map((s) => s.trim()).filter(Boolean) }) }} className="w-full bg-luxury-900 border border-luxury-600 rounded px-3 py-2 text-white text-sm" /></div>
              ...
              <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.has_security_option} onChange={(e) => setForm({ ...form, has_security_option: e.target.checked })} className="accent-luxury-gold" /> Has Security Option</label>
              ...
              <button onClick={handleSave} className="w-full bg-luxury-gold text-luxury-900 py-2.5 rounded font-semibold hover:bg-luxury-gold-light transition-colors mt-2">{editing ? 'Update Apartment' : 'Add Apartment'}</button>
            </div>
          </div>
        </div>
      )}
