    ...
    <>
      <section className="pt-24 pb-12 bg-luxury-900">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h1 className="text-4xl sm:text-5xl font-bold mb-4">Our Luxury <span className="text-luxury-gold">Fleet</span></h1>
      ...
      <section className="pb-20 bg-luxury-900">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-wrap gap-2 mb-8 justify-center">
            {categories.map((c) => (
              <button key={c} onClick={() => setFilter(c)} className={`px-4 py-2 rounded text-sm font-medium transition-colors ${filter === c ? 'bg-luxury-gold text-luxury-900' : 'bg-luxury-800 text-white/70 hover:bg-luxury-700'}`}>{c}</button>
            ))}
          </div>
