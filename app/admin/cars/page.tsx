  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={() => { setForm(emptyForm); setEditing(false); setShowForm(true) }} className="bg-[#F8AC28] text-[#0a0e13] px-4 py-2 rounded font-semibold hover:bg-[#e69d1f] transition-colors">+ Add Car</button>
      </div>
      <div className="bg-[#0e1117] border border-[#2a2d33] rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          <thead><tr className="bg-[#1a1d23] text-white/70"><th className="text-left p-3">Name</th><th className="text-left p-3">Price</th><th className="text-left p-3">Category</th><th className="text-left p-3">Available</th><th className="text-right p-3">Actions</th></tr></thead>
          <tbody>{cars.map((car) => (
            <tr key={car.id} className="border-t border-[#2a2d33]"><td className="p-3 font-medium">{car.name}</td><td className="p-3 text-white/70">{car.price}</td><td className="p-3"><span className="bg-[#F8AC28]/20 text-[#F8AC28] text-xs px-2 py-0.5 rounded">{car.category}</span></td><td className="p-3">{car.available ? <span className="text-green-400">Yes</span> : <span className="text-red-400">No</span>}</td><td className="p-3 text-right"><button onClick={() => handleEdit(car)} className="text-[#F8AC28] hover:text-[#F8AC28]-light mr-3">Edit</button><button onClick={() => handleDelete(car.id)} className="text-red-400 hover:text-red-300">Delete</button></td></tr>
          ))}</tbody>
        </table>
      </div>
      {showForm && (
          ...
          <div className="bg-[#0e1117] border border-[#2a2d33] rounded-xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-xs text-white/70 mb-1">HP</label><input type="number" value={form.hp} onChange={(e) => setForm({ ...form, hp: Number(e.target.value) })} className="w-full bg-[#0a0e13] border border-[#2a2d33] rounded px-3 py-2 text-white text-sm" /></div>
                <div><label className="block text-xs text-white/70 mb-1">Seats</label><input type="number" value={form.seats} onChange={(e) => setForm({ ...form, seats: Number(e.target.value) })} className="w-full bg-[#0a0e13] border border-[#2a2d33] rounded px-3 py-2 text-white text-sm" /></div>
              </div>
              <div><label className="block text-xs text-white/70 mb-1">Engine</label><input value={form.engine} onChange={(e) => setForm({ ...form, engine: e.target.value })} className="w-full bg-[#0a0e13] border border-[#2a2d33] rounded px-3 py-2 text-white text-sm" /></div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-xs text-white/70 mb-1">Transmission</label><select value={form.transmission} onChange={(e) => setForm({ ...form, transmission: e.target.value })} className="w-full bg-[#0a0e13] border border-[#2a2d33] rounded px-3 py-2 text-white text-sm"><option>Auto</option><option>Manual</option></select></div>
                <div><label className="block text-xs text-white/70 mb-1">Category</label><select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} className="w-full bg-[#0a0e13] border border-[#2a2d33] rounded px-3 py-2 text-white text-sm"><option>Luxury</option><option>Sports</option><option>Electric</option><option>SUVs</option></select></div>
              </div>
              ...
              <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.has_driver_option} onChange={(e) => setForm({ ...form, has_driver_option: e.target.checked })} className="accent-[#F8AC28]" /> Has Driver Option</label>
              ...
              <button onClick={handleSave} className="w-full bg-[#F8AC28] text-[#0a0e13] py-2.5 rounded font-semibold hover:bg-[#e69d1f] transition-colors mt-2">{editing ? 'Update Car' : 'Add Car'}</button>
            </div>
          </div>
        </div>
      )}
